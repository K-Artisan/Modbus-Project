...
namespace Azir.Modbus.Protocol.FuncitonNum.CustomerRequest
{
    ...
    /// </typeparam>
    public class FunNum06CustomerRequestData<T> : IGenericCustomerRequestData<FunNum06RequestDataBase, T>
    {
        ...
        public FunNum06CustomerRequestData()
        {
        ...
        public List<FunNum06RequestDataBase> CovertToFunNumRequestDataBases()
        {
            List<FunNum06RequestDataBase> funNum06RequestDataBases = new List<FunNum06RequestDataBase>();
            ...
            for (int i = 0, k = 0; i <= countOfRequestDataBase && k < bytePresetData.Length; i++, k += 2)
            {
                ...
                {
                    //HL　HL
                    funNum06RequestDataBase.PresetDataFirst = bytePresetData[k + 1]; //第一个字节被赋值bytePresetData的高位
                    funNum06RequestDataBase.PresetDataSecond = bytePresetData[k];
                }
                else
                {
                    //LH LH
                    funNum06RequestDataBase.PresetDataFirst = bytePresetData[k];    //第一个字节被赋值bytePresetData的低位
                    funNum06RequestDataBase.PresetDataSecond = bytePresetData[k + 1];
                }
